...
**⚠️ `manages_rep` was rewritten 2026-08-19 and now carries three rules, all verified by impersonation.** (1) **Orphan routing:** a rep with no `assigned_manager_id` is reviewable by **any** sales_manager plus management, not management alone — there was no pre-existing rule to reuse, nothing referenced `assigned_manager_id IS NULL` before this. (2) **The management branch no longer requires the target to be `role='rep'`**: three live plans were permanently unapprovable because their author used the tester role-switch back to `management`, so `manages_rep` went false for their own submitted plans. The **sales_manager branch keeps** the `role='rep'` test, or the orphan fallback would let one manager read another manager's rows. (3) **`p_rep <> auth.uid()` — nobody manages themselves.** This is load-bearing: without it, dropping the role test for management would have made **self-approval possible**, silently destroying the invariant the whole PJP design rests on. Every table using the function has its own self-read path, so excluding self costs no visibility.

**Manager reads are scoped to their own reps.** `store_visits`, `attendance`, `challans` and `challan_items` previously used a bare `get_my_role() = ANY(['sales_manager','management'])`, so a sales_manager could read **every** rep's rows — measured live at 28 of 28 visits and 20 of 20 attendance rows while owning one rep. They now read `get_my_role() = 'management' or manages_rep(...)`. The management branch is tested **first and separately** on purpose: `manages_rep` requires `role='rep'`, so relying on it alone would hide rows by non-rep users (a switched tester, or a null `user_id`) from management too. Consequence to expect the first time a sales_manager account exists: Store Detail's "Visits & Notes", the legacy dashboard and `rep-report-detail` all narrow for them.
...
**Reps never see or receive prices.** The rep's product fetch omits the price columns entirely, and `trg_snapshot_order_item_price` (BEFORE INSERT on `order_items`) fills `price_per_case`/`price_per_bottle` from the catalog, **overwriting whatever the client sent**. So order value is server-derived (catalog price x rep quantities), the rep device never holds pricing, and a tampered client cannot dictate a price — verified live: a line claiming ₹1 stored ₹15000. It runs after `trg_reject_oos_order_item` (same timing, alphabetical, `'r' < 's'`), so an OOS line is refused first. *Limit worth knowing:* every app user shares the `authenticated` Postgres role, and column privileges are per-role, so prices cannot be column-revoked for reps only without moving management's read path behind a definer view.
...
### Stock snapshots — two buckets (shelf + godown)
...
Stock is **two buckets per product**: `shelf_*` (everything in the store — on the shelves and in back stock) and `godown_*` (the store's own godown), each a `_cases`/`_bottles` pair. Floor and Display were separate until 2026-08-07; the field said they are the same thing, so they merged.

**The merge is FORWARD, never retroactive.** The ledger is append-only, so pre-merge rows keep their `floor_*`/`display_*` values and are summed on READ by **`shelfFigure()`**, which returns `{ value, legacy }`. `legacy: true` means the number is two older readings added together and was **never taken as a single shelf count** — Store Detail says so explicitly ("Shelf figure combines the older separate floor and display counts"). Same honesty rule as the unattributed ledger rows: present what the data is, never imply more precision. `SNAPSHOT_COLUMNS` is the one column list every reader uses so no caller forgets the legacy pair. **`cases`/`bottles` remain the authoritative TOTAL** and are what every pre-existing reader uses (`useStores`, `useManagementDashboard`) — the buckets are the breakdown, so nothing downstream had to change.

**ABSENT IS NOT ZERO.** A bucket left blank is `null` ("no godown / not counted"); an explicit `0` means "counted and empty". Many stores have no godown, and a forced 0 would later read as real data. A bare `check (col >= 0)` admits NULL, which is how the `>= 0` discipline coexists with this. Prefill deliberately leaves never-recorded buckets blank so "no godown" doesn't drift into "godown is empty". Rows predating buckets entirely carry a total with every bucket null and render **"Breakdown not recorded"** — never a guessed split. Rows from the floor/display era are a *different* case: they do have a split, and it is summed forward into shelf and labelled as merged.

Math lives in **`lib/stockBuckets.ts`** (pure, `lib/stockBuckets.test.ts`): totals are computed **in whole bottles then converted**, same discipline as `inventoryMath`, so shelf 3cs+30btl is 4cs+6btl, not a drifted 3cs+30btl.
...
- **derived** — far-from-store (reuses `distance_from_store_meters`), impossible-movement (coords+timestamps already present), off-plan, plan-not-approved, duplicate-store, and **no-work-recorded**.

**`no_work_recorded` is the "time in store" signal, and it is deliberately PRESENCE, not duration.** The obvious design — span the work artifacts a visit produced (`last_artifact_ts − first_artifact_ts`) — **cannot work on the current write path**: the stepper buffers photos and stock locally and flushes them in one burst inside `handleCheckOut`, so every artifact lands within seconds of check-out. Measured against live data, artifact spans were 0.00–0.54 min against tap gaps up to 14 min; the span would read ~0 for every visit and flag every honest rep. So **duration stays `store_visits.duration_minutes`** (the client-clock tap gap) and the honesty signal is whether the visit produced any work at all — a rep at home cannot fake a live shelf photo of that specific shop. Artifacts counted: `store_visit_photos`, `store_stock_snapshots`, `orders`, `challans` (all by `visit_id`), plus a non-empty `store_visits.notes`. Three carve-outs keep it quiet: `artifact_count` undefined means *uncounted, not zero*; a still-open visit has produced nothing *yet*; and an `auto_closed` row already carries its own flag, so a flat battery earns one flag, not two. **If true time-in-store is ever needed, the unlock is committing artifacts as they happen, not a smarter query.** Deriving is deliberate: an approved plan retroactively clears its visits, and **a client that skips a check is still caught** because the signal comes from the data, not from whether a dialog was shown.
...
Rebuilt 2026-08-19 as a virtualized **`SectionList`** with sticky headers and count pills, in three labelled sections — **Odometer flags · Location & visit flags · Day plans** — plus pull-to-refresh via `usePullToRefresh`. The families need different judgements, so mixing them into one stream made the manager re-orient at every card.

- **Freshness:** only plans within **`PLAN_ACTIONABLE_DAYS = 7`** (`planCutoffDate()`) are offered for approval; the query was previously unbounded. Stale ones stay in history (`journey_plans` has no DELETE grant) and are surfaced as a **count** in the empty state — a bare "nothing waiting" over a silently dropped pile would tell a manager they were finished when they were not. The Team badge counts actionable plans only.
- **The queue excludes plans the viewer submitted.** The read policy allows `rep_id = auth.uid()`, but nobody manages themselves, so your own plan can never be approved by you. Listing them is what made three live plans look permanently broken.
- **Perf:** `store_visits(check_in_time desc)`, `store_visits(user_id)`, `attendance(user_id, check_in_time desc)`, `store_stock_snapshots(visit_id)`, `orders(visit_id)`, `challans(visit_id)` and a partial `journey_plans(submitted_at) where status='submitted'` all back queue filters that were previously sequential scans. The store name-map fetched the **entire** stores table and is now `.in('id', storeIds)`; three sequential round-trips collapsed into one parallel wave; visits are capped at `VISIT_SCAN_LIMIT = 500` ordered **newest-first then reversed** (an ascending limit would keep the oldest rows and hide today's flags, and the movement check needs chronological order).
- **Failed review reports the truth.** A 0-row update reads the row back and says *"Already approved by <name>"*, "this is your own plan", or "not assigned to you" instead of guessing between all three.
...
⚠️ **Realtime channel topics must be unique per hook instance** (`journey-plans-${useId()}`, matching `locreq-${repId}`). Team and the review queue both mount `usePlanSubmissions`, and a native stack keeps Team mounted underneath. `RealtimeClient.channel(topic)` **dedupes** — a second call returns the *same* channel, so a shared topic does NOT throw; what it does is let the queue's unmount `removeChannel` the shared instance and **silently kill Team's subscription**. (This was once misdiagnosed as the cause of an app crash. It is not — verified by reading the client source.) Manager notification reuses the **live-location Realtime pattern** (`journey_plans` is the second table in `supabase_realtime`) rather than push notifications — no new native module, so PJP stays OTA-shippable.

### Delivery challans (rep capture + manual entry)
`challans` (`store_id`, nullable `visit_id`, `challan_date`, nullable `challan_number`, `photo_path`, `recorded_by`, `notes`, `created_at`) + `challan_items` (`challan_id`, `product_id`, `qty_qts`/`qty_pints`/`qty_nips`). Screen: `app/(rep)/challan.tsx`, pushed from the rep Dashboard stack; helpers in `lib/challan.ts` (pure, `lib/challan.test.ts`); data in `hooks/useChallans.ts`.

**Auto-OCR is deliberately OUT of scope.** Many challans are handwritten in mixed English/Hindi/Devanagari on carbon paper, which no free offline recogniser reads reliably — so the manual rail *is* the product, and the priority is to start collecting structured data. The seam for a future **printed-challan** extractor (reusing the permit text-layer pipeline, which has no OCR either) is the shape of `ChallanLine` plus the header fields; nothing assumes the values were typed rather than parsed.

- **Quantities are loose BOTTLE counts by beer-trade size class** (Qt 650 ml · Pint 330 ml · Nip 180 ml), transcribed straight off the challan. **No cases conversion:** `qty_per_carton` is per *product* but the three columns are three *sizes*, so dividing a mixed total by one carton size would invent a number.
- `product_id` is **NOT NULL** — only catalog products are recorded; competitor lines on the same challan are ignored. `sortOwnBrandFirst()` floats our line to the top via the `OWN_BRAND` regex (a heuristic over free text that only decides **sort order**, so a miss costs a scroll, never a blocked entry; upgrade path is an `is_own_brand` column).
- **DB-enforced:** `challan_items_nonempty` refuses an all-zero line (23514) and `unique (challan_id, product_id)` refuses a double entry (23505).
- **Correctable by design** — UPDATE exists here, unlike the permit/ledger tables, because correction is the whole point of a manual rail. `recorded_by` is pinned so a row can't be reassigned. **No DELETE:** the photo is evidence, so a challan is corrected, never removed. Consequence: a header whose lines fail to insert is an orphan, recovered by `ChallanLinesError` carrying the id so "Save" retries only the lines (the same shape as management's Add-User "Retry Save").
...
**Sending happens entirely inside Postgres** — trigger `trg_notify_plan_submitted` → `notify_plan_submitted()` → `net.http_post` (pg_net) → `https://exp.host/--/api/v2/push/send`. An Edge Function was ruled out: they run on the **caller's JWT** (the anon-key-only invariant), so a rep's client could not read a manager's token, and the workaround — a definer RPC returning tokens granted to `authenticated` — would let any signed-in client **harvest every device token in the system**.

- **⚠️ NO FCM CREDENTIAL EXISTS IN THIS APP OR DATABASE.** The Expo push token *is* the credential; Expo relays to FCM using the service-account key held in **EAS credentials only**. It must never reach the repo, the database, or Supabase Vault. Vault currently holds **zero** secrets and the trigger reads none — verified. (Only if Expo's optional *Enhanced Security for Push* is ever enabled does an **Expo access token** — not the FCM key — go into Vault.)
- **`token` is UNIQUE on its own, not `(user_id, token)`** — a handset maps to exactly one person, so a shared device makes the token **move**, never duplicate. `register_push_token` is the **only** legal writer (no INSERT/UPDATE policy at all, same posture as `orders`); a user may only SELECT and DELETE their own row. **Not even management can read another user's token** — a token is a capability, and anyone holding it can push to that device.
- **Unregister must run BEFORE `signOut()`**: the delete policy is `user_id = auth.uid()`, so after sign-out the row is unreachable and the next person holding the phone would keep receiving the previous user's notifications.
- **Routing mirrors §3.4** — designated manager if the rep has one, otherwise **all sales_managers + management**; the submitter is always excluded. It **cannot call `manages_rep`** (that asks "can *this caller* manage X" and needs `auth.uid()`; a trigger has no caller), so the rule is written twice and must be kept in step by hand.
- Fires only on *entry* into `submitted` (editing an already-submitted plan doesn't re-buzz) and is **fire-and-forget** — a push failure never blocks a rep from submitting. Payload is minimal: title, "<Rep> submitted a plan for <date>", `data.planId`. Nothing that shouldn't sit on a lock screen.

### Auto-checkout at 22:30 IST
`public.auto_close_stale()` (SECURITY DEFINER, no app-role EXECUTE) closes any `store_visits` or `attendance` row left open, scheduled by **pg_cron job `auto-close-stale`**.

⚠️ **`cron.timezone` on this project is `GMT`, not Asia/Kolkata** (verified live). The schedule is therefore **`'0 17 * * *'` = 17:00 UTC = 22:30 IST**. Writing `'30 22 * * *'` would fire at **04:00 IST** and close legitimately-open evening visits — the exact failure to avoid. The function computes its own IST cutoff independently, so the schedule only decides *when to look*; India has no DST.

Each row is closed at **22:30 IST of its own check-in day**, not "today 22:30" — so a visit left open on 31 Jul is closed at 31 Jul 22:30, not stamped with a 7-day duration. That also makes the one-time backfill and the nightly run literally the same code path.

`auto_closed boolean` marks them. `duration_minutes` stays NULL (we don't know when they left), no position is invented, and `odo_end`/`total_distance_km` stay NULL so an auto-closed day contributes **zero** to travel allowance. Because `check_out_time` is set, resume (`useOpenVisit`) and the stepper's check-in lookup both ignore them with no code change — they filter `check_out_time is null`. Surfaced in the exception queue as a **soft** flag (a flat battery, not dishonesty).
...
**OTA vs build:** JS-only changes ship with `eas update --branch preview --environment preview` and land on any installed build whose `runtimeVersion` matches. A **new EAS build is required** only when a native module is added/changed — currently `react-native-maps`, `react-native-svg`, `react-native-reanimated` (+ `react-native-worklets`), `expo-camera/location/secure-store/sharing/document-picker/print/updates/blur/haptics/linear-gradient/file-system`, `expo-speech-recognition`, **`@infinitered/react-native-mlkit-text-recognition`**, **`expo-notifications`**, **`expo-device`**.

⚠️ **Bump `app.json` `version` whenever a native module is added.** `runtimeVersion.policy` is `appVersion`, so leaving it unchanged lets a later `eas update` push JS onto an older binary that lacks the new native module — `requireNativeModule` then throws at import and white-screens the app. The ML Kit addition is why the app moved **1.0.0 → 1.1.0**, and `expo-notifications`/`expo-device`/`expo-constants` is why it moved **1.1.0 → 1.2.0**.

**Push build prerequisites** (all done as of 2026-08-19 except the EAS upload): `app.json` → `android.googleServicesFile: "./google-services.json"`, and that file **is committed on purpose** — EAS builds from the git archive, so gitignoring it means no FCM config at build time. The **service-account key** (`*firebase-adminsdk*.json`) is gitignored by glob — the glob matters because a regenerated key downloads with a different random suffix — and goes to `eas credentials` → Android → *Push Notifications: FCM V1*, nowhere else. `android.package` is still the `expo prebuild` placeholder `com.anonymous.TankAssistCodex` and is now baked into the Firebase app registration; changing it later means re-registering in Firebase, a fresh `google-services.json`, and a new app identity existing installs won't upgrade into.
